...
export async function getAdminOverview() {
    ...
    const clock = await pool.query(`select base_real_at,base_world_day,world_days_per_real_day,updated_at from world_clock where id=1`).catch(() => ({ rows: [] }));
    return {
        summary: summary.rows[0],
        origins: origins.rows,
        slots: slots.rows,
        locations: locations.rows,
        funnel: funnel.rows.map(({ label, count }) => ({ label, count })),
...
export async function getAdminPlayer(playerId) {
    ...
    if (!life)
        return { player: playerResult.rows[0], life: null };
    const [skills, inventory, flags, knowledge, rumors, biography, analytics, travel, combat, openMove, openActivity] = await Promise.all([
        ...
        life.active_travel_id ? pool.query(`select id,status,current_node_id,health_start,health_current,run_flags,visited_nodes,updated_at from travel_runs where id=$1`, [life.active_travel_id]) : Promise.resolve({ rows: [] }),
        life.active_combat_id ? pool.query(`select id,status,turn_no,player_hp,enemy_hp,enemy_state,enemy_intent,combat_flags,updated_at from combat_sessions where id=$1`, [life.active_combat_id]) : Promise.resolve({ rows: [] }),
    ...
    const inventoryMap = new Map(inventory.rows.map((x) => [x.item_id, Number(x.quantity)]));
    const itemCatalog = [...content.items.values()].map((item) => ({ id: item.id, name: item.name, description: item.description, quantity: inventoryMap.get(item.id) ?? 0 }));
    return {
...
export async function updateAdminLife(lifeId, patch) {
    return inTransaction(async (client) => {
        ...
        if (health > maxHealth)
            throw new HttpError(400, 'invalid_health', 'Health cannot exceed max health');
        const money = patch.money ?? Number(life.money);
        if (life.phase === 'OPEN_LIFE' && patch.currentWorldDay !== undefined)
            throw new HttpError(400, 'world_day_read_only', 'OPEN_LIFE age is controlled by the shared world clock');
...
export async function updateAdminSkill(lifeId, skillId, level) {
    await inTransaction(async (client) => {
        const exists = await client.query(`select id from lives where id=$1 and alive=true`, [lifeId]);
        if (!exists.rows[0])
            throw new HttpError(404, 'life_not_found', 'Active life not found');
        if (level === 0)
            await client.query(`delete from life_skills where life_id=$1 and skill_id=$2`, [lifeId, skillId]);
        else
            await client.query(`insert into life_skills(life_id,skill_id,level) values($1,$2,$3) on conflict(life_id,skill_id) do update set level=excluded.level`, [lifeId, skillId, level]);
...
export async function updateAdminInventory(lifeId, itemId, quantity) {
    if (!content.items.has(itemId))
        throw new HttpError(400, 'unknown_item', 'Unknown item');
    await inTransaction(async (client) => {
        const exists = await client.query(`select id from lives where id=$1 and alive=true`, [lifeId]);
        if (!exists.rows[0])
            throw new HttpError(404, 'life_not_found', 'Active life not found');
        if (quantity === 0)
            await client.query(`delete from life_inventory where life_id=$1 and item_id=$2`, [lifeId, itemId]);
        else
            await client.query(`insert into life_inventory(life_id,item_id,quantity) values($1,$2,$3) on conflict(life_id,item_id) do update set quantity=excluded.quantity`, [lifeId, itemId, quantity]);
        await client.query(`update lives set state_version=state_version+1,updated_at=now() where id=$1`, [lifeId]);
