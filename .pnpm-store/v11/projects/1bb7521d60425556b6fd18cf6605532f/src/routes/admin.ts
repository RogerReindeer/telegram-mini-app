...
import { getReaderDeliveryOverview, retryFailedReaderCoinEvent } from '../services/readerCoinDelivery.js';

export const adminRouter = Router();

type Attempt = { count: number; windowStart: number; blockedUntil: number };
...
adminRouter.post('/login', asyncRoute(async (req, res) => {
  ...
  if (!adminPassword) throw new HttpError(503, 'admin_not_configured', 'Admin password is not configured');
  const key = req.ip || req.socket.remoteAddress || 'unknown';
  const now = Date.now();
  let attempt = attempts.get(key) ?? { count: 0, windowStart: now, blockedUntil: 0 };
  if (attempt.blockedUntil > now) throw new HttpError(429, 'admin_login_blocked', 'Too many login attempts');
  if (now - attempt.windowStart > WINDOW_MS) attempt = { count: 0, windowStart: now, blockedUntil: 0 };
...
adminRouter.get('/commerce/reader-deliveries', asyncRoute(async (_req, res) => {
  res.json(await getReaderDeliveryOverview());
}));
adminRouter.post('/commerce/reader-deliveries/:eventId/retry', asyncRoute(async (req, res) => {
  const { eventId } = z.object({ eventId: z.string().uuid() }).parse(req.params);
  res.json(await retryFailedReaderCoinEvent(eventId));
}));
