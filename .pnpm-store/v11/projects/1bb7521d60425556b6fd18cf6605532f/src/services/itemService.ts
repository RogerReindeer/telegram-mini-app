...
export type EquipmentSlot = 'outfit'|'outerwear'|'head'|'footwear'|'main_hand'|'off_hand'|'belt'|'carry';
export type ItemCondition = 'good'|'worn'|'damaged'|'broken';
export type StoryKind = 'normal'|'critical'|'evidence'|'temporary';
...
export interface ItemInstanceView {
  instanceId:string;
  id:string;
  name:string;
  description?:string;
  ...
  storyKind:StoryKind;
  protected:boolean;
  locationType:'CARRIED'|'HOME'|'EQUIPPED'|'WORKPLACE'|'ACTIVITY';
...
function def(itemId:string):ItemDefinition {
  const value=content.items.get(itemId) as ItemDefinition|undefined;
  if(!value) throw new Error(`Unknown item ${itemId}`);
  return value;
}

export async function addItem(client:DbClient,life:LifeRow,itemId:string,quantity:number,originData:Record<string,unknown>={}) {
  if(quantity<=0)return;
  const item=def(itemId);
  if(item.instance){
    for(let i=0;i<quantity;i++){
      await client.query(
        `insert into life_item_instances(life_id,item_id,location_type,condition,story_kind,protected,origin_data,acquired_world_day)
         values($1,$2,'CARRIED','good',$3,$4,$5,$6)`,
        [life.id,itemId,item.storyKind??'normal',Boolean(item.protected),originData,life.current_world_day]
      );
    }
    return;
  }
  await client.query(
    `insert into life_inventory(life_id,item_id,quantity) values($1,$2,$3)
     on conflict(life_id,item_id) do update set quantity=life_inventory.quantity+excluded.quantity`,
    [life.id,itemId,quantity]
  );
}

export async function removeItem(client:DbClient,life:LifeRow,itemId:string,quantity:number) {
  if(quantity<=0)return;
  const item=def(itemId);
  if(item.instance){
    const rows=await client.query(
      `select i.id,i.protected from life_item_instances i
    ...
    const removable=rows.rows.filter((r:any)=>!r.protected).slice(0,quantity);
    if(removable.length<quantity) throw new HttpError(409,'not_enough_items',`Not enough ${itemId}`);
    await client.query(`delete from life_item_instances where id=any($1::uuid[])`,[removable.map((r:any)=>r.id)]);
    return;
  }
  ...
  if(next<0) throw new HttpError(409,'not_enough_items',`Not enough ${itemId}`);
  if(next===0) await client.query(`delete from life_inventory where life_id=$1 and item_id=$2`,[life.id,itemId]);
  else await client.query(`update life_inventory set quantity=$3 where life_id=$1 and item_id=$2`,[life.id,itemId,next]);
}

export async function ensureBaseEquipment(client:DbClient,life:LifeRow){
  for(const [slot,itemId] of [['outfit','simple_clothes'],['footwear','work_shoes']] as Array<[EquipmentSlot,string]>){
    const equipped=await client.query(`select 1 from life_equipment where life_id=$1 and slot=$2`,[life.id,slot]);
    if(equipped.rows[0]) continue;
...
export async function equipInstance(client:DbClient,life:LifeRow,instanceId:string,slot:EquipmentSlot){
  ...
  if(!instance) throw new HttpError(404,'item_not_found','Item not found');
  if(instance.location_type!=='CARRIED'&&instance.location_type!=='EQUIPPED') throw new HttpError(409,'item_not_carried','Item is not carried');
  const item=def(String(instance.item_id));
  if(!(item.equipSlots??[]).includes(slot)) throw new HttpError(409,'slot_unavailable','This item cannot be equipped there');
  ...
  await client.query(
    `insert into life_equipment(life_id,slot,item_instance_id) values($1,$2,$3)
...
export async function equipFirstByItemId(client:DbClient,life:LifeRow,itemId:string,slot:EquipmentSlot){
  ...
  if(!r.rows[0]) throw new HttpError(409,'item_not_found',`Missing ${itemId}`);
  await equipInstance(client,life,String(r.rows[0].id),slot);
}

export async function unequipSlot(client:DbClient,life:LifeRow,slot:EquipmentSlot){
...
export async function moveInstanceHome(client:DbClient,life:LifeRow,instanceId:string){
  if(life.current_location_id!=='home_qinghe'&&life.current_location_id!=='qinghe') throw new HttpError(409,'home_storage_unavailable','Home storage is only available at home');
  ...
  if(!r.rows[0]) throw new HttpError(404,'item_not_found','Item not found');
  if(r.rows[0].location_type==='EQUIPPED'){
    ...
    if(slot==='outfit'||slot==='footwear') throw new HttpError(409,'base_clothing_required','Базовую одежду нельзя оставить в сундуке, пока она на вас');
...
export async function retrieveInstanceHome(client:DbClient,life:LifeRow,instanceId:string){
  if(life.current_location_id!=='home_qinghe'&&life.current_location_id!=='qinghe') throw new HttpError(409,'home_storage_unavailable','Home storage is only available at home');
  ...
  if(!r.rows[0]) throw new HttpError(404,'item_not_found','Stored item not found');
}

export async function storeStack(client:DbClient,life:LifeRow,itemId:string,quantity:number){
  if(life.current_location_id!=='home_qinghe'&&life.current_location_id!=='qinghe') throw new HttpError(409,'home_storage_unavailable','Home storage is only available at home');
  if(quantity<=0) throw new HttpError(400,'invalid_quantity','Invalid quantity');
  const r=await client.query(`select quantity from life_inventory where life_id=$1 and item_id=$2 for update`,[life.id,itemId]);
  const have=Number(r.rows[0]?.quantity??0); if(have<quantity) throw new HttpError(409,'not_enough_items','Not enough items');
  if(have===quantity) await client.query(`delete from life_inventory where life_id=$1 and item_id=$2`,[life.id,itemId]);
  else await client.query(`update life_inventory set quantity=$3 where life_id=$1 and item_id=$2`,[life.id,itemId,have-quantity]);
...
export async function retrieveStack(client:DbClient,life:LifeRow,itemId:string,quantity:number){
  if(life.current_location_id!=='home_qinghe'&&life.current_location_id!=='qinghe') throw new HttpError(409,'home_storage_unavailable','Home storage is only available at home');
  if(quantity<=0) throw new HttpError(400,'invalid_quantity','Invalid quantity');
  ...
  const have=Number(r.rows[0]?.quantity??0); if(have<quantity) throw new HttpError(409,'not_enough_items','Not enough stored items');
  if(have===quantity) await client.query(`delete from life_storage where life_id=$1 and storage_id='home_chest' and item_id=$2`,[life.id,itemId]);
  else await client.query(`update life_storage set quantity=$3 where life_id=$1 and storage_id='home_chest' and item_id=$2`,[life.id,itemId,have-quantity]);
...
export async function equipmentMap(client:DbClient,lifeId:string){
  ...
  const out:Record<string,ItemInstanceView|null>={outfit:null,outerwear:null,head:null,footwear:null,main_hand:null,off_hand:null,belt:null,carry:null};
...
function originText(originData:any):string|null {
  if(!originData||typeof originData!=='object')return null;
  if(typeof originData.note==='string')return originData.note;
  if(typeof originData.source==='string')return `Получено: ${originData.source}`;
  return null;
}

export async function inventoryViews(client:DbClient,lifeId:string){
  ...
  const inventory:Array<StackItemView|ItemInstanceView>=[];
  for(const row of stacks.rows){const item=def(String(row.item_id));inventory.push({id:item.id,name:item.name,description:item.description,quantity:Number(row.quantity),storyKind:item.storyKind??'normal',protected:Boolean(item.protected),tags:item.tags??[]});}
  for(const row of instances.rows){const item=def(String(row.item_id));inventory.push({instanceId:String(row.id),id:item.id,name:item.name,description:item.description,quantity:1,condition:row.condition,storyKind:row.story_kind,protected:Boolean(row.protected),locationType:row.location_type,locationKey:row.location_key,equippedSlot:row.equipped_slot??null,equipSlots:item.equipSlots??[],originText:originText(row.origin_data)});}
  const storage:Array<StackItemView|ItemInstanceView>=[];
  for(const row of storedStacks.rows){const item=def(String(row.item_id));storage.push({id:item.id,name:item.name,description:item.description,quantity:Number(row.quantity),storyKind:item.storyKind??'normal',protected:Boolean(item.protected),tags:item.tags??[]});}
  for(const row of storedInstances.rows){const item=def(String(row.item_id));storage.push({instanceId:String(row.id),id:item.id,name:item.name,description:item.description,quantity:1,condition:row.condition,storyKind:row.story_kind,protected:Boolean(row.protected),locationType:'HOME',locationKey:'home_chest',equippedSlot:null,equipSlots:item.equipSlots??[],originText:originText(row.origin_data)});}
  return {inventory,equipment,storage};
}

export async function equippedItemIds(client:DbClient,lifeId:string){
  ...
  return new Map<EquipmentSlot,string>(r.rows.map((x:any)=>[x.slot,String(x.item_id)]));
}

export async function visibleEquipmentDescription(client:DbClient,lifeId:string){
  ...
  return r.rows.map((x:any)=>def(String(x.item_id)).visibleWhenEquipped).filter((x):x is string=>Boolean(x)).slice(0,4);
}

export async function createWorkpiece(client:DbClient,life:LifeRow,itemId:string,workplace:string,activityId:string){
...
export async function removeWorkpiece(client:DbClient,life:LifeRow,itemId:string,workplace:string){
